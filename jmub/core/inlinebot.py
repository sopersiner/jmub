...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧⵧⵧDRAKONⵧⵧⵧⵧⵧ⍣**](https://t.me/PP_GX)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**jmub"):
            ...
            result = builder.article(
                title="jmub Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب دراكون**",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/PP_GX"),
                Button.url(
                    "المطور",
                    "t.me/PP_MX",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل دراكون نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[DRAKON](https://t.me/PP_GX)",
