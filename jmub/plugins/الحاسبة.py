...
from PP_GX import PP_GX

# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @PP_GX  ~ @RR7PP
...
@PP_GX.ar_cmd(pattern="الحاسبة(?:\s|$)([\s\S]*)")
async def icalc(e):
    if e.client._bot:
        return await e.reply("**الحاسبة العلمية لسورس دراكون\n @PP_GX**", buttons=lst)
    results = await e.client.inline_query(Config.TG_BOT_USERNAME, "calc")
    await results[0].click(e.chat_id, silent=True, hide_via=True)
    await e.delete()


@PP_GX.tgbot.on(InlineQuery)
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحاسبة العلمية لسورس دراكون\n @PP_GX**", buttons=lst
        )
        await event.answer([calc])


# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @PP_GX  ~ @RR7PP
@PP_GX.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @PP_GX  ~ @RR7PP
@PP_GX.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحاسبة العلمية لسورس دراكون\n @PP_GX**", buttons=lst)
