...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"تم حظرك بنجاح لن تم تحذيرك وبعد شنو اسويلك حب اعذرني ❤️"
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""❃ اهلا بك {mention} \n حب حاليا ماموجود من اجي عود ارسلك لاتكرر رسائل لن تبلع حظر ❤️
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق دراكون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@jmub.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق دراكون
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق دراكون
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية دراكون.txt",
        caption="قائمة المسموح لهم الحالية\n سورس دراكون العربي \n @jmthon",
    )  # ترجمه وكتابة فريق دراكون
مه وكتابة فريق دراكون
دراكون
