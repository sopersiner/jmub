...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس دراكون " + "**• حدث خطأ ما :**\n"
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**• سورس دراكون محدث الى أخر اصدار**"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس دراكون"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس دراكون أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/sopersiner/temp"
    os.chdir("/app")
    try:
