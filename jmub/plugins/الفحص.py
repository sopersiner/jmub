...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس دراكون يعمل بنجاح "
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس دراكون يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار دراكون :** `{jmthonversion}`\n"
...
ROZ_T = (
    f"**⌯︙بوت دراكون يعمل بنجاح 🤍،**\n"
    f"**   - اصدار التليثون :** `1.23.0\n`"
    f"**   - اصدار دراكون :** `4.0.0`\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jmub.uid:
            buttons = [
                [
                    Button.url("قنـاة السـورس ⚙️", "https://t.me/PP_GX"),
                    Button.url("المطـور 👨🏼‍💻", "https://t.me/PP_MX"),
                ]
            ]
